...
select c.`nomeCliente` as Cliente, DATE_FORMAT(
...
select c.`nomeCliente` as Cliente, COUNT(*) as 'Total alugado'
from aluguel a
    join cliente c on a.`idCliente` = c.`idCliente`
GROUP BY
    c.`nomeCliente`;
...
select sum(ae.`valorItem`) as 'Total arrecadado'
from aluguelequipamento ae;

/*11. Listar o nome dos equipamentos com quantidade em estoque maior que 40*/
select e.`nomeEquipamento` as Equipamento, e.qtd as Quantidade
from equipamento e
WHERE
    e.qtd > 40;
...
select c.`nomeCliente` as Cliente, sum(ae.`valorItem`) as 'Valor Pago'
from
    aluguel a
    join aluguelequipamento ae on a.`idAluguel` = ae.`idAluguel`
    join cliente c on a.`idCliente` = c.`idCliente`
...
select e.`nomeEquipamento` as Equipamento, e.`valorHora` as Valor
from equipamento e
WHERE
    e.`valorHora` > (
...
select f.`nomeFuncionario` as Funcionário
from aluguel a
    join funcionario f on a.`idFuncionario` = f.`idFuncionario`
GROUP BY
    f.`nomeFuncionario`
...
select DATE_FORMAT(
        a.`dataHoraRetirada`, '%d/%m/%Y'
    ) as 'Data Aluguel', sum(ae.`valorItem`) as Total
from
...
select a.`formaPagamento` as 'Forma de Pagamento', sum(ae.`valorItem`) as Total
from
    aluguel a
    join aluguelequipamento ae on a.`idAluguel` = ae.`idAluguel`
WHERE
    a.`formaPagamento` is NOT null
GROUP BY
    a.`formaPagamento`;

/*20. Exibir o nome dos equipamentos alugados mais de 3 vezes.*/
select e.`nomeEquipamento` Equipamento, COUNT(*) as Total
from
    aluguelequipamento ae
    join equipamento e on ae.`idEquipamento` = e.`idEquipamento`
GROUP BY
