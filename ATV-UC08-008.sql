...
select sum(ae.`valorItem`) as 'Total arrecadado' from aluguelequipamento ae;
...
select c.`nomeCliente` as Cliente, sum(ae.`valorItem`) as 'Valor Pago' from aluguel a
join aluguelequipamento ae on a.`idAluguel` = ae.`idAluguel`
join cliente c on a.`idCliente` = c.`idCliente`
GROUP BY c.`nomeCliente`;
...
select DATE_FORMAT(a.`dataHoraRetirada`, '%d/%m/%Y') as 'Data Aluguel', sum(ae.`valorItem`) as Total from aluguelequipamento ae
join aluguel a on ae.`idAluguel` = a.`idAluguel`
GROUP BY ae.`idAluguel`
HAVING Total > 100;

/*19. Mostrar o total de valor pago por forma de pagamento.*/
select a.`formaPagamento` as 'Forma de Pagamento', sum(ae.`valorItem`) from aluguel a
join aluguelequipamento ae on a.`idAluguel` = ae.`idAluguel`
