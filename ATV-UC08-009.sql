...
select f.`nomeFuncionario` as Funcionário, DATE_FORMAT(a.`dataHoraRetirada`, '%d/%m/%Y') as 'Data do Aluguel', c.`nomeCliente` as Cliente from aluguel a
...
insert into aluguelequipamento(idEquipamento, idAluguel, valorItem, valorUnitario, qtd) values(equipamento, aluguel, valor_total, valor_unitario, quantidade)
